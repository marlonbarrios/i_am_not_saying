...
let instructionText = 'Press SPACE to generate text, then click and drag or pinch (thumb + index) to draw.';
...
// Hand tracking (pinch-to-draw)
let handTrackerVideo = null;
...
const translations = {
  'en': {
    ...
    instruction: 'Select language, press SPACE to generate text, then click and drag or pinch to draw',
    hint: 'Move vertically to control volume • Speed affects playback rate and font size',
    generatingText: 'Generating text...',
    generatingAudio: 'Generating audio...',
    textReady: 'Text ready • Click and drag or pinch to draw',
...
const sketch = p => {
  p.setup = function() {
    ...
    p.textAlign(p.LEFT);
    p.fill(0);
    // Initialize hand tracker (camera); may prompt for permission
    initHandTracker().then(({ video }) => {
      if (video) {
        handTrackerVideo = video;
  ...
  p.draw = function() {
    ...
    const videoReady = handTrackerVideo && handTrackerVideo.readyState >= 1 && handTrackerVideo.videoWidth > 0;
    if (showVideoAsBackground && videoReady) {
      p.push();
      p.translate(p.width, 0);
      p.scale(-1, 1);
      // Use canvas drawImage so video frames always render (p5.image can be unreliable with video)
      const ctx = p.drawingContext;
      if (ctx && typeof ctx.drawImage === 'function') {
        ctx.drawImage(handTrackerVideo, 0, 0, handTrackerVideo.videoWidth, handTrackerVideo.videoHeight, 0, 0, p.width, p.height);
      } else {
        p.image(handTrackerVideo, 0, 0, p.width, p.height);
      }
      p.pop();
    } else if (showVideoAsBackground && handTrackerVideo) {
    ...
    // Update hand tracker and compute draw source (mouse or pinch)
    if (handTrackerVideo && handTrackerReady) {
      updateHandTracker(handTrackerVideo, p.width, p.height, true);
    }
    const hand = getHandState();
    const isPinching = hand.isPinching;
    const mouseDrawing = p.mouseIsPressed && p.mouseButton === p.LEFT;
    const isDrawing = (mouseDrawing || isPinching) && textReady && letters.length > 0 && !showHomePage;
    const drawX = isPinching ? pinchToCanvas(hand.pinchX, hand.pinchY, p.width, p.height, true).x : p.mouseX;
    const drawY = isPinching ? pinchToCanvas(hand.pinchX, hand.pinchY, p.width, p.height, true).y : p.mouseY;
    ...
    if (isDrawing && !wasDrawingLastFrame) {
      ...
      prevMouseX = drawX;
      prevMouseY = drawY;
      // Start/resume audio when drawing starts (mouse or pinch) so you hear the text as you draw
    ...
    // Hide cursor when drawing (mouse or pinch), show cross otherwise
    if (isDrawing) {
      p.noCursor();
    ...
    // Draw new letters if text is ready and (mouse drag or hand pinch)
    if (isDrawing) {
      hasStartedDrawing = true;

      // Update volume and playback speed from draw position (mouse or hand)
      updateVolumeFromY(p, drawX, drawY);
      ...
      // Just stopped drawing (mouse released or pinch released) — pause audio
